...
static AVFormatContext *file_iformat = NULL;
...
static int packet_queue_init(PacketQueue *q)
{
    ...
    if (!q->mutex) {
        printf("[%s:%d] bruce >>> SDL_CreateMutex(): %s\n",__func__, __LINE__, SDL_GetError());
    ...
    if (!q->cond) {
        printf("[%s:%d] bruce >>> SDL_CreateCond(): %s\n", __func__, __LINE__, SDL_GetError());
...
{
    MyAVPacketList *pkt, *next_pkt;

    SDL_LockMutex(q->mutex);
    for (pkt = q->first_pkt; pkt; pkt = next_pkt) {
        next_pkt = pkt->next;
...
#if 0 //rtsp和mmsh协议暂时不需要 CONFIG_RTSP_DEMUXER || CONFIG_MMSH_PROTOCOL
        if (is->paused &&
...
static VideoState *stream_open(const char *filename, AVInputFormat *iformat)
{
    ...
    if (startup_volume < 0)
        printf("[%s:%d] bruce >>> -volume=%d < 0, setting to 0\n",__func__, __LINE__, startup_volume);
        //av_log(NULL, AV_LOG_WARNING, "-volume=%d < 0, setting to 0\n", startup_volume);
    if (startup_volume > 100)
        printf("[%s:%d] bruce >>> -volume=%d > 100, setting to 100\n",__func__, __LINE__, startup_volume);
    ...
    if (!is->read_tid) {
        printf("[%s:%d] bruce >>> SDL_CreateThread(): %s\n",__func__, __LINE__, SDL_GetError());
...
static int realloc_texture(SDL_Texture **texture, Uint32 new_format, int new_width, int new_height, SDL_BlendMode blendmode, int init_texture)
{
    ...
    if (!*texture || SDL_QueryTexture(*texture, &format, &access, &w, &h) < 0 || new_width != w || new_height != h || new_format != format) {
        ...
        printf("[%s:%d] bruce >>> Created %dx%d texture with %s.\n",__func__, __LINE__, new_width, new_height, SDL_GetPixelFormatName(new_format));
...
static void video_audio_display(VideoState *s)
{
    ...
    if (s->show_mode == SHOW_MODE_WAVES) {
        ...
        if (!s->rdft || !s->rdft_data){
            printf("[%s:%d] bruce >>> Failed to allocate buffers for RDFT, switching to waves display\n", __func__, __LINE__);
...
int main(int argc, char **argv)
{
    ...
    if (argc != 2) {
        printf("the num of params is wrong! pls input ffplay input_file!\n");
        return -1;
    }
    ...
    int SDL_WINDOW_flags = SDL_WINDOW_HIDDEN;
    if (alwaysontop)
#if SDL_VERSION_ATLEAST(2,0,5)
        SDL_WINDOW_flags |= SDL_WINDOW_ALWAYS_ON_TOP;
#else
        printf("Your SDL version doesn't support SDL_WINDOW_ALWAYS_ON_TOP. Feature will be inactive.\n");
    ...
    window = SDL_CreateWindow(program_name, SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, default_width, default_height, SDL_WINDOW_flags);
